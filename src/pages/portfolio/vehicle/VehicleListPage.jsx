...
import {
    Car, Plus, Search, ChevronRight, Edit2, Trash2,
    RefreshCw, Fuel, Calendar, Ruler, ChevronDown, X, Image,
} from "lucide-react"
...
function VehicleRow({ item, onEdit, onDelete, onStatusChange, onImage }) {
    const [statusOpen, setStatusOpen] = useState(false)
    const status   = STATUS_MAP[item.status] || STATUS_MAP.available
    const f        = item.features || {}
    const coverUrl = item.cover_url || item.images?.[0]?.url || item.images?.[0]
    ...
    return (
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors group">
            <div className="relative w-10 h-10 flex-shrink-0">
                {coverUrl ? (
                    <img src={coverUrl} alt={item.title} className="w-10 h-10 rounded object-cover border border-gray-200" />
                ) : (
                    <div className="w-10 h-10 rounded bg-purple-50 border border-purple-100 flex items-center justify-center text-purple-700">
                        <Car size={16} />
                    </div>
                )}
            ...
            <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-gray-800 truncate group-hover:text-purple-700 transition-colors">{item.title}</p>
                <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 mt-0.5">
                    {f.marka && <span className="text-[10px] text-gray-400 font-medium">{f.marka}</span>}
                    {f.model && <span className="text-[10px] text-gray-400 font-medium">{f.model}</span>}
                    {f.yil   && <span className="text-[10px] text-gray-400 font-medium flex items-center gap-0.5"><Calendar size={9} />{f.yil}</span>}
                    {f.km    && <span className="text-[10px] text-gray-400 font-medium flex items-center gap-0.5"><Ruler size={9} />{f.km}</span>}
                    {f.yakit && <span className="text-[10px] text-gray-400 font-medium flex items-center gap-0.5"><Fuel size={9} />{f.yakit}</span>}
                </div>
            </div>
          ...
          {item.moderation_status === "rejected" ? "Reddedildi" : "İncelemede"}
                    ...
                    <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded shadow-lg z-10 overflow-hidden min-w-[120px]">
                        {Object.entries(STATUS_MAP).map(([key, val]) => (
                            <button key={key} onClick={() => { onStatusChange(item.id, key); setStatusOpen(false) }}
                                    className={`w-full text-left px-3 py-2.5 text-[10px] font-bold hover:bg-gray-50 transition-colors ${
                                        item.status === key ? "text-purple-700" : "text-gray-700"
                                    }`}>
                                {val.label}
                            </button>
                        ))}
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
                ...
                <OfferStatsButton item={item} accent="purple" />
            </div>
        </div>
    )
}
...
export default function VehicleListPage() {
    ...
    const fetchCounts = async () => {
        try {
            const res = await api.get("/agent/portfolio", { params: { type: "vasita", per_page: 1000 } })
    ...
    const fetchItems = async (reset = false) => {
        ...
        try {
            const res = await api.get("/agent/portfolio", {
                params: { type: "vasita", status: statusFilter || undefined, page: reset ? 1 : page }
            })
    ...
    const handleDelete = async (id) => {
        if (!confirm("Bu aracı portföyden silmek istediğinize emin misiniz?")) return
        try {
            const removedStatus = items.find(i => i.id === id)?.status
            await api.delete(`/agent/portfolio/${id}`)
            setItems(prev => prev.filter(i => i.id !== id))
            setCounts(c => ({ ...c, total: c.total - 1, [removedStatus]: Math.max(0, (c[removedStatus] || 0) - 1) }))
            toast({ message: "Araç silindi." })
    ...
    const handleStatusChange = async (id, status) => {
        try {
            ...
            setCounts(c => ({ ...c, [prevStatus]: Math.max(0, (c[prevStatus] || 0) - 1), [status]: (c[status] || 0) + 1 }))
    ...
    const filtered = items.filter(i =>
        ...
        i.features?.marka?.toLowerCase().includes(search.toLowerCase()) ||
        i.features?.model?.toLowerCase().includes(search.toLowerCase())
    )
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <h1 className="text-xl font-bold text-gray-800">Vasıta Portföyü</h1>
                        </div>
                        <p className="text-gray-400 text-xs font-medium">Satışa çıkardığınız araçları yönetin.</p>
                    </div>
                    <PortfolioAddMenu current="vehicle" />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div className="md:col-span-1 flex flex-col gap-4 h-full">
                        ...
                        <div className="bg-gray-900 border border-gray-800 rounded-sm shadow-sm p-4 text-white flex-1 flex flex-col">
                            <p className="text-[9px] font-bold uppercase tracking-wider text-purple-400 mb-1.5">Pazaryeri</p>
                            <h3 className="font-bold text-sm text-white leading-snug mb-1">Yeni araç ekle</h3>
                            <p className="text-gray-400 text-[10px] font-medium leading-relaxed mb-4">
                                Stoğunuza yeni araç ekleyin, gelen taleplere otomatik eşleşin.
                            </p>
                            <Link to="/portfolio/vehicle/add"
                                  className="flex items-center justify-center gap-1.5 bg-gradient-to-r from-purple-700 to-indigo-600 hover:from-purple-800 hover:to-indigo-700 text-white px-4 py-2.5 rounded font-bold text-xs uppercase tracking-wider transition-all mt-auto">
                        ...
                        <div className="bg-white border border-gray-200 rounded-sm shadow-sm overflow-hidden">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 py-3 border-b border-gray-200 bg-gray-50">
                                <div className="relative flex-1">
                                    ...
                                    <input type="text" placeholder="Marka, model veya başlık ara..."
                                ...
                                <div className="flex items-center gap-1 bg-gray-100 p-0.5 rounded border border-gray-200 flex-shrink-0 flex-wrap">
                                    {[
                                        { key: "",          label: `Tümü (${counts.total})` },
                                        { key: "available", label: `Satışta (${counts.available})` },
                                        { key: "reserved",  label: `Rezerve (${counts.reserved})` },
                                        { key: "sold",      label: `Satıldı (${counts.sold})` },
                                    ].map(f => (
                                ...
                                <button onClick={() => fetchItems(true)} className="w-8 h-8 flex items-center justify-center rounded text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors flex-shrink-0">
                                    <RefreshCw size={13} />
                                </button>
                            </div>
                            ...
                            {loading && items.length === 0 ? (
                                ...
                                <div className="text-center py-16 px-4">
                                    ...
                                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Araç Bulunamadı</p>
                                    <p className="text-[10px] text-gray-400 font-medium mt-1 mb-4">
                                        {search ? "Arama kriterlerinize uygun araç yok." : "Henüz portföyünüze araç eklemediniz."}
                                    </p>
                                    <Link to="/portfolio/vehicle/add"
                                ...
                                <div>
                                    {filtered.map(item => (
                                        <VehicleRow key={item.id} item={item}
                                                    onEdit={id => navigate(`/portfolio/vehicle/${id}/edit`)}
